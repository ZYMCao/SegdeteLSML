[New Video Object Segmentation and Tracking for SAM 2](https://humansignal.com/blog/video-object-segmentation-and-tracking-with-videovector-tag-for-sam-2-in-label-studio/)
...
Follow the steps below to wrap custom machine learning model code with the Label Studio ML SDK, or see [our library of example ML backends](https://labelstud.io/guide/ml_tutorials) to integrate with popular machine learning frameworks and tools such as [Huggingface’s Transformers](https://huggingface.co/docs/transformers/index) , [OpenAI](https://openai.com/) , [Langchain](https://www.langchain.com/) and others.

For information on using one of Label Studio’s example backends, see [Set up an example ML backend](https://labelstud.io/guide/ml#Set-up-an-example-ML-backend) .
...
- - for an interactive labeling scenario
- `predictions`: [Predictions array in JSON format](https://labelstud.io/guide/export#Raw-JSON-format-of-completed-tasks)
...
Complete the following steps:1. Define an inference call with the `predict()` method as outlined above. The `predict()` method takes task data and context data:
...
2. Return a result in the [Label Studio predictions format](https://labelstud.io/guide/predictions#Format-pre-annotations-for-Label-Studio) , which varies depending on the type of labeling being performed.

Refer to the code example linked above for more details about how this might be performed for a NER labeling project.

For more information about enabling pre-annotations, see [Interactive pre-annotations](https://labelstud.io/guide/ml#Interactive-pre-annotations) .
...
By default, the `fit` method is called at any data action in Label Studio, like creating a new task or updating annotations. You can modify this behavior in using [Webhooks](https://labelstud.io/guide/webhooks) .
...
- `data`: The payload received from the event (see the [Webhook event reference](https://labelstud.io/guide/webhook_reference) ).
...
By default, the payload of the webhook event does not contain the annotation itself. You can either [modify the webhook event](https://labelstud.io/guide/webhooks) sent by Label Studio to send the full payload, or retrieve the annotation using the Label Studio API using the [get annotation by its ID endpoint](https://labelstud.io/api#operation/api_annotations_read) , [SDK](https://labelstud.io/guide/sdk) using the [get task by ID method](https://labelstud.io/sdk/project.html#label_studio_sdk.project.Project.get_task) , or by retrieving it from [target storage that you set up](https://labelstud.io/guide/storage) to store annotations.

See the [annotation webhook event reference](https://labelstud.io/guide/webhook_reference#Annotation-Created) for more details about the webhook event.
...
If your data is stored in a cloud, local directory, or has been imported into Label Studio, you will need to set the `LABEL_STUDIO_URL` and `LABEL_STUDIO_API_KEY` environment variables.For more information, see [Allow the ML backend to access Label Studio data](https://labelstud.io/guide/ml#Allow-the-ML-backend-to-access-Label-Studio-data) .
...
The ML backend server is available at `http://localhost:9090`. You can use this URL when [connecting the ML backend to Label Studio](https://labelstud.io/guide/ml#Connect-the-model-to-Label-Studio) .
...
You can use the API or **Settings \> Model**. For more information, see [Connect the model to Label Studio](https://labelstud.io/guide/ml#Connect-the-model-to-Label-Studio) .

### In this article

1. [1. Install the ML backend repo](https://labelstud.io/guide/ml_create#1-Install-the-ML-backend-repo)
2. [2. Create an empty ML backend](https://labelstud.io/guide/ml_create#2-Create-an-empty-ML-backend)
3. [3. Implement prediction logic](https://labelstud.io/guide/ml_create#3-Implement-prediction-logic)
4. [Support interactive pre-annotations in your ML backend](https://labelstud.io/guide/ml_create#Support-interactive-pre-annotations-in-your-ML-backend)
5. [4. Implement training logic (optional)](https://labelstud.io/guide/ml_create#4-Implement-training-logic-optional)
6. [Trigger training with webhooks](https://labelstud.io/guide/ml_create#Trigger-training-with-webhooks)
7. [Other methods and parameters](https://labelstud.io/guide/ml_create#Other-methods-and-parameters)
8. [4. Ensure the ML backend can access Label Studio data](https://labelstud.io/guide/ml_create#4-Ensure-the-ML-backend-can-access-Label-Studio-data)
9. [5. Run the ML backend server](https://labelstud.io/guide/ml_create#5-Run-the-ML-backend-server)
10. [Run without Docker](https://labelstud.io/guide/ml_create#Run-without-Docker)
11. [Modify the host and port](https://labelstud.io/guide/ml_create#Modify-the-host-and-port)
12. [Test your ML backend](https://labelstud.io/guide/ml_create#Test-your-ML-backend)
13. [6. Connect the ML backend to Label Studio](https://labelstud.io/guide/ml_create#6-Connect-the-ML-backend-to-Label-Studio)

Designed for teams of all sizes [Compare Versions](https://humansignal.com/pricing)